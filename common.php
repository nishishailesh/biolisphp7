...
function main_menu()
{
	...
	<li><a href="#">Documents/Records</a>
			<ul>
				...
				<li><a href=ljn.php>LJ N</a>
				<li><a href=import_qcn.php>Import QC N</a>
...
{
		$link=start_nchsls();
		$all_sample_details=get_sample($sample_id);
		
		$sql='select * from examination where sample_id=\''.$sample_id.'\'';
		$result=mysql_query($sql,$link);
		echo '<table border=1 bgcolor=lightblue>';
		echo '<tr><td colspan=3 align=left>-----['.$sample_id.']-----<td>';
		echo '<tr><td colspan=3 style="background:pink; color:blue;">'.$all_sample_details['sample_type'].'/'.$all_sample_details['preservative'].'</td></tr>';
...
function get_sample($sample_id)
{
	$link=start_nchsls();
	$sql_sample_examination='select * from	sample	where sample_id=\''.$sample_id.'\' ';
	
	if(!$result_sample_examination=mysql_query($sql_sample_examination,$link)){echo mysql_error();return FALSE;}
	return $array_sample=mysql_fetch_assoc($result_sample_examination);
}
...
function autoverify($sample_id,$filename,$action)
{
	...
	foreach ($all_details  as $key=>$value)
	{
		...
		{			
			...
			append_if_not('Some results may be / may not be absurd. Correlate clinically and repeat sample.',$sample_id,1007);
...
//4
			...
			$this->Cell(59, $h=0, $txt='Sex/Age: '.$sample_array['sex'].'/'.$sample_array['age'], $border, $ln=0, $align='L', $fill=false, $link='', $stretch=1, $ignore_min_height=false, $calign='T', $valign='M');
			$this->SetFont('courier', '', 10);
			$this->SetXY(15+60,$y_counter);
...
//5
			...
			//$this->Cell(44, $h=0, $txt=' Printed: '.strftime('%Y-%m-%d %H:%M:%S'), $border, $ln=0, $align='L', $fill=false, $link='', $stretch=1, $ignore_min_height=false, $calign='T', $valign='M');
				...
				$this->Cell($w=40, $h=0, 'Examination(Test)',$border, $ln=0, $align='', $fill=false, $link='', 
					$stretch=1, $ignore_min_height=false, $calign='T', $valign='M');
				...
				$this->Cell($w=40, $h=0, 'Reference range',$border, $ln=0, $align='', $fill=false, $link='', 
					$stretch=1, $ignore_min_height=false, $calign='T', $valign='M');
...
function prepare_ri_str($ex_id,$sample_id)
{
	...
	if($ex_id==4 )
	{
		creatinine_serum_reference_interval($xml,$sample_data);
		//echo $GLOBALS['ri_str'];
	...
	if($ex_id==21 )
	{
		uric_acid_serum_reference_interval($xml,$sample_data);
		//echo $GLOBALS['ri_str'];
...
function print_report_pdf_A4_ri_last($sample_id_array,$doctor)
{
	$acr_check_code=array(	'-1'=>'',
					'-2'=>'',
					'-3'=>'',
