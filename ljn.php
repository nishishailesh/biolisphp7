...
function draw_qc($equipment_name,$year,$month,$day,$hour,$select,$code,$all_examinations,$normal_qc_id,$abnormal_qc_id,$lol_qc_id)
{
	...
	echo '<tr><th colspan=3>LJ Chart for '.$equipment_name.'</th></tr>
			<tr><td></td><th> Normal QC-'.$normal_qc_id.'</th><th>Abnormal QC-'.$abnormal_qc_id.'</th><th>Abnormal QC-'.$abnormal_qc_id.'</th></tr>';
	//QYYMMDDHH
	if($select=='Year')
	{
		$normal_qc_from=
	...
	else if($select=='Month')
	{
		...
		$abnormal_qc_to=$abnormal_qc_from + 3124;
                ...
                $lol_qc_from=
                                                ...
                                                + ($year-2000)*1000000
                                                +       $month*10000;
                $lol_qc_to=$abnormal_qc_from + 3124;
			...
			where  
					...
					sample_id='.($i+$abnormal_qc_id*100000000);
                        ...
                        where
                                        equipment_name=\''.$equipment_name.'\' 
                                        and
                                        sample_id='.($i+$lol_qc_id*100000000);
	}
...
////SMP
                $result_qc_lol=mysql_query($sql_qc_lol,$link);
                echo mysql_error();
...
//////////
               ...
               echo '<td>';
                        while($array_qc_lol=mysql_fetch_assoc($result_qc_lol))
                        {
                                if($array_qc_lol['sd']>0)
                                {
                                        $title= $array_qc_lol['sample_id'].','.
                                                        $array_qc_lol['code'].','.
                                                        $array_qc_lol['result'].',sd obtained='.
                                                        round((($array_qc_lol['result']-$array_qc_lol['target'])/$array_qc_lol['sd']),1).',target='.
                                                        $array_qc_lol['target'].',sd='.
                                                        $array_qc_lol['sd'];

                                if($array_qc_lol['comment']=='-1')
                                        {
                                                $code_display_ab='<font color=red>'.str_pad($array_qc_lol['code'],4,'_').'</font>';
                                        }
                                        else
                                        {
                                                $code_display_ab=str_pad($array_qc_lol['code'],4,'_');
                                        }
                                        $use_qc=        $array_qc_lol['equipment_name'].'|'.
                                                                $array_qc_lol['sample_id'].'|'.
                                                                $array_qc_lol['repeat_id'].'|'.
                                                                $array_qc_lol['time_data'].'|'.
                                                                $array_qc_lol['code'];
